...
int main(int argc, char *argv[]) {
    ...
    // 配置EL1809的DC参数（作为参考时钟）
    ecrt_slave_config_dc(sc_el1809, 0x0300, 1000000, 0, 0, 0); // 1ms周期

    // 配置EL2809的DC参数（跟随参考时钟）
    ecrt_slave_config_dc(sc_el2809, 0x0300, 1000000, 500000, 0, 0); // 0.5ms偏移

    // 设置EL1809为参考时钟
    ecrt_master_select_reference_clock(master, sc_el1809);
    ...
    while (run) {
        ...
        uint64_t app_time = (uint64_t)ts.tv_sec * 1000000000ULL + ts.tv_nsec;
